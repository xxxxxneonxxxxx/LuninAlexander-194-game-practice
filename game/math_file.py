...
def chis(strok, stol, prav, lev):
    ...
    def stroka(prav, lev,
               strok):  # формирование строк: prav-самое миниммальное значение, lev самое максимальное значение, a - нужно, strok количество строк в игре
        chis = []  # числа в сумме дающие sum
        if strok == 5:
    ...
    def play(strok, prav, lev):  # random strok
        # strok- количество столбов x - левая граница   y - правая граница prav - правое значение чисел (обычно 1 или 2), lev - левое значение чисел (обычно 4,10,19)
        chis = stroka(prav, lev, strok)
        for i in range(10):  # сортировка строки меняя метами индексы
            rand1 = random.randint(0, len(chis) - 1)
            rand2 = random.randint(0, len(chis) - 1)
            b = chis[rand1]  # значение chis[rand1] в массиве
            chis[rand1] = chis[rand2]
            chis[rand2] = b  # 10 раз переставляем числа чтобы зарандомить их
        return chis
